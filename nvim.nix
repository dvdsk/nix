...
programs.neovim = {
	...
	(nvim-treesitter.withPlugins (p: with p; [
        bash c cpp python rust lua zig toml ini json json5 jq regex query
        make ninja dot nix html css typescript javascript markdown markdown-inline
        git-config git-rebase gitcommit gitignore comment udev passwd
        typst latex
	]))
